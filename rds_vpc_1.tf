resource "aws_db_subnet_group" "psql-subnet-vpc-1" {
  name        = "psql-subnet-vpc-1"
  description = "RDS subnet group"
  subnet_ids  = [aws_subnet.vpc-1-sub-b.id, aws_subnet.vpc-1-sub-bb.id]
}

resource "aws_db_parameter_group" "psql-parameters-vpc-1" {
  name        = "psql-parameters-vpc-1"
  family      = "postgres12"
...
# 3: psql -h psql-db-vpc-1.c9fowi1eofeq.us-east-1.rds.amazonaws.com -d psql -U cnc
# 4: \l

resource "aws_db_instance" "test-psql-vpc-1" {
  allocated_storage       = 100 # 100 GB of storage, gives us more IOPS than a lower number
  engine                  = "postgres"
  #engine_version          = "12.2"
  engine_version          = "12.5"
  instance_class          = "db.m4.large" # use micro if you want to use the free tier
  identifier              = "psql-db-vpc-1"
  name                    = "psql"
  username                = "cnc"            # username
  password                = var.RDS_PASSWORD # password
  db_subnet_group_name    = aws_db_subnet_group.psql-subnet-vpc-1.name
  parameter_group_name    = aws_db_parameter_group.psql-parameters-vpc-1.name
